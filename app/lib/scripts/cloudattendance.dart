...
class CloudAttendance {
  ...
  Future<Attendance> getAttendanceData() async {
    ...
    DatabaseReference _ref = _database.ref().child("users").child(id);
    _ref.keepSynced(true);
    await _ref.once().then((snapshot) {
      attendance = Attendance.fromDataSnapshot(snapshot.snapshot);
      return;
    }, onError: (e) {
      throw (e);
    });
  ...
  deleteAllDataOnCloudAndDevice() async {
    try {
      ...
      await _realtimedb.reference().child("users").child(id).remove();
      FirebaseFirestore _firestrore = await FirebaseFirestore.instance;
      // _firestrore.settings(timestampsInSnapshotsEnabled: true);
      await _firestrore.collection('users').doc(id).delete();
